function goodsSelect(goodsUrl, itemsUrl) {
    ...
    var createList = function () {
        for (var i = 0, n = pageItems.length, html = ''; i < n; i++) {
            ...
            html += '<b>' + pageItems[i].saleNumShow + '</b>';
            html += '</li>';
        }
        return html;
    };
    ...
    var createItems = function () {
        for (var i = 0, n = spItems.length, html = ''; i < n; i++) {
            ...
            html += '<img src="' + spItems[i].image + '" />';
            html += '<h5>' + spItems[i].title + '</h5>';
            html += '<p><span>¥' + spItems[i].salePrice + '</span><span>库存：' + spItems[i].saleNumShow + '</span></p>' +
                '<input type="hidden" name="num" id="num" value="' + spItems[i].saleNumShow + '">';
            html += '<b data-id="' + spItems[i].id + '">删除</b>';
            html += '</li>';
        }
        return html;
    };
