...
import com.b2c.oms.utils.PddApiUtils;
...
public class AjaxOrderPddController {
    ...
    @RequestMapping(value = "/pull_order_test", method = RequestMethod.GET)
    public DeferredResult<String> deferredResultReq () throws Exception  {
        ...
        myThreadPoolTaskExecutor.execute(new Runnable() {
            ...
            public void run(){
                ...
                while(i <= 10){
                    ...
                    //返回结果
                    result.setResult("DeferredResult!!"+i);
                }
            }
        });
       return result;
    ...
    @RequestMapping(value = "/pull_order_test1", method = RequestMethod.GET)
    public WebAsyncTask<String> getOrderListTest(HttpServletRequest req)
            throws Exception {
        boolean error = false;
        ...
        // 指定10s的超时
        WebAsyncTask<String> webTask = new WebAsyncTask<>(10000, callable);
...
//                // 任务处理时间5s,不超时
//                Thread.sleep(5 * 1000L);
//                return "任务"+(i-1)+"顺利执行成功！任何异常都没有抛出！";
//            }
//
//            return "所有任务顺利执行成功！任何异常都没有抛出！";
//
//        });
//
//
...
//        return asyncTask;
    ...
    public ApiResult<ErpSalesPullCountResp> getOrderList(@RequestBody DataRow reqData, HttpServletRequest req)
            throws Exception {
        ...
        for (int i = 0; i < forSize; i++) {
            ...
            result = this.pullPddOrder(clientId, clientSecret, accessToken, pageIndex, pageSize, startTime1, endTime1,
                    shopId);
            pageIndex++;
            ...
            while (pageIndex <= totalPage) {
                ...
                // log.info("开始更新第"+pageIndex+"页");
                // log.info("查询到数据:"+result.getData().getTotalRecords());
                result = this.pullPddOrder(clientId, clientSecret, accessToken, pageIndex, pageSize, startTime1,
                        endTime1, shopId);
                if (result.getCode() == 0) {
                    updCount += result.getData().getUpdCount();
                    insertCount += result.getData().getAddCount();
     ...
     * @throws Exception
     */
    private ApiResult<ErpSalesPullCountResp> pullPddOrder( String clientId,String clientSecret,String accessToken,Integer pageNo, Integer pageSize, Long startTime, Long endTime, Integer shopId) throws Exception {
    ...
    public static String extractIndex(String encryptedData) {
        ...
        String[] parts = com.pdd.pop.sdk.common.util.StringUtils.split(encryptedData, sepInData);
    ...
    @RequestMapping(value = "/decrypt_pdd_order", method = RequestMethod.POST)
    public void decryptPddData() {
        String clientId = "dc953bcf16d24b27abf3e64a59e1ecd1";
        String Secret = "de296599e194a08cbfbb2b3b340e11fec7a1bacc";
        String accessToken = "f63a24e7801e43b6927923f5382ef63b153ddafa";
        PopClient client = new PopHttpClient(clientId, Secret);
        try {
            PddOpenDecryptMaskBatchRequest request = new PddOpenDecryptMaskBatchRequest();
            List<PddOpenDecryptMaskBatchRequest.DataListItem> dataList = new ArrayList<>();

            PddOpenDecryptMaskBatchRequest.DataListItem item = new PddOpenDecryptMaskBatchRequest.DataListItem();
            item.setDataTag("210109-392456440623883");
            item.setEncryptedData(
                    "~AgAAAACRbyMHyJFZ+ADRJ51QAWDIUKFzRrbQ3C9nEgr4oHY1q03Kfp9fx2TyAdDKSGQpdFNqRHVoGAKbUh+50SOpCnh9kfKg2xOIzdgRYVA=~qEJbXbJzOyYT2nI0lozVM4+ugFdC84nSe96oqq5I0ZxYkqfZvgAlp7NGfGJbenc0T+F78txB~0~~");
            dataList.add(item);
    ...
    @RequestMapping(value = "/order_item_del", method = RequestMethod.POST)
    public ApiResult<Integer> delPddOrderItemIsGift(@RequestBody Long orderItemId, HttpServletRequest req) {
        orderPddService.delPddOrderItemIsGift(orderItemId);
...
/*        var dfgnShop = shopService.getShop(5);
     ...
     * @return
     */
    @RequestMapping(value = "/pdd_order_cancel", method = RequestMethod.POST)
    public ApiResult<String> pdd_order_cancel(@RequestBody DataRow reqData) {
        var result = orderPddService.cancelOrderPrint(reqData.getLong("orderId"));
        return new ApiResult<>(result.getCode(), result.getMsg());
    }
